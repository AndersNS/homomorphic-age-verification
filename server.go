...
type Server struct {
	...
	// that a verification is in flight so the server can validate the
	// decryption request.
...
func NewServer(params bgv.Parameters, currentDate time.Time, ageThreshold uint64, secretKey *rlwe.SecretKey) (*Server, error) {
	...
	return &Server{
		...
		ageThreshold: uint64(ageThreshold),
		cks:          cks,
...
// in the request. It computes (maxBirthDays - encryptedBirthDays), giving an
...
func (s *Server) VerifyAge(request *ClientRequest) (*ServerResponse, error) {
	...
	thresholdDate := s.currentDate.AddDate(-int(s.ageThreshold), 0, 0)
	...
	// Compute (maxBirthDays - birthDays) homomorphically.
	...
	// Add maxBirthDays to get Enc(maxBirthDays - birthDays).
...
// CompleteVerification receives the client's blinded ciphertext and decryption
// share, combines it with the server's own share to decrypt the blinded result,
...
// server only sees r * (maxBirthDays - birthDays). This preserves the sign
...
func (s *Server) CompleteVerification(req *DecryptionRequest) (*Attestation, error) {
	...
	// We use a convention: values in [0, p/2] represent non-negative results
	// (user is old enough), and values in (p/2, p) represent negative results
	// (too young). This works because our values are small relative to p,
