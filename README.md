...
The server needs to verify that a user is at least 18 years old, but the user doesn't want to reveal their exact birth date. And the server doesn't want the client to know what the age requirement is. Neither party can decrypt alone -- both must contribute decryption shares to reveal the computation result.
...
sequenceDiagram
    ...
    S->>S: Compute Enc(requiredBirthday - birthDays)
...
3. **Server** computes `requiredBirthday - Enc(birthDay)` homomorphically. Returns the encrypted difference and a session ID. The ciphertext stays encrypted under the collective key — the server cannot decrypt it alone.
...
- **Client** cannot learn the exact age requirement — the server's response is an encrypted difference under the collective key, which the client cannot decrypt alone. The client computes `reqBlind` from a conservative public bound (worst-case: person born at epoch verified today) so it doesn't leak the threshold.
